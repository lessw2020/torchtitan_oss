...
class save_on_cpu_orig(saved_tensors_hooks):
...
class manage_activations(saved_tensors_hooks):
    ...
    def __init__(self, pin_memory: bool = False, device_type: str = "cuda") -> None:
        ...
        self.tracker = {} # track what saved/offloaded/compressed tensors, are where
        self.mem_offload_cache = {} # cache of available memory for tensors
        ...
        def get_num_bytes_tensor(self, x: torch.Tensor) -> int:
            # get the number of bytes in a tensor, for memory management purposes
            return x.element_size() * x._base_storage().nbytes()
        
        def get_num_bytes_shape(self, x: torch.Tensor) -> int:
            # get the number of bytes in a tensor's shape (e.g., for memory management purposes), assuming that we are storing it as a list of tensors
            return 8 * len(x._base_storage().shape()) + sum([get_num_bytes_tensor(t) for t in x._base_storage().shape()])
        ...
        def unpack_tensor(tensor_id: str) -> torch.Tensor:
