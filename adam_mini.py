                    ...
                    if ("embed_tokens" in name or "wte" in name or "lm_head" in name):
                        ...
                        if (len(state) == 0):
                            ...
                            if (self.world_size > 1) and (self.model_sharding is True):
                                ...
                                if (s >= 2):
                                    reduced = True
                                #print(f"{dimension=}, {reduced=}, {tensor_list=}")
                        ...
                        if (state["reduced"]):
                            #print(f"pre_all_reduce {tmp_lr=}")
                            #lr_local = tmp_lr.to_local()
                            #dist.all_reduce(lr_local, op=dist.ReduceOp.SUM)
                            #tmp_lr = lr_local
                            #tmp_lr.redistribute(placements=[Replicate()])
                            #assert tmp_lr == lr_local, f"reduce mismatch {tmp_lr=}, {lr_local=}"
                            #print(f"post all_reduce {tmp_lr=}")
                            pass
