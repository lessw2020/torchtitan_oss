...
class save_on_cpu(saved_tensors_hooks):
    ...
    def __init__(self, pinned_memory=True, device_type="cuda"):
        ...
        def aam_pack_hook(input_tensor: torch.Tensor) -> str:
            ...
            if input_tensor.numel() < min_copy_size:
                #print(f"skipping {input_tensor.shape=}, {input_tensor.dtype=}")
                gpu_clone = input_tensor.clone()
                fast_lookup[tensor_id] = (gpu_clone, prev, 0, -1)  # -1 means no side stream
                prev = tensor_id
                return tensor_id
            ...
            fast_lookup[tensor_id] = (cpu_tensor, prev, -1, all_current_stream_idx)
